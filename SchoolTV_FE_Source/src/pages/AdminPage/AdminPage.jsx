  ...
  useEffect(() => {
    ...
    const fetchPaymentHistory = async () => {
      try {
        ...
        const data = await response.json();
        const totalRevenue = data.$values.reduce((total, payment) => total + payment.amount, 0);
    ...
    const fetchUserHistory = async () => {
      try {
        const response = await apiFetch('accounts/admin/statistics/user-history', {
          headers: {
            'accept': '*/*'
          }
        });
      ...
      <Layout style={{ minHeight: '90vh' }}>
        ...
        <Layout style={{ padding: '0 24px 24px' }}>
          ...
          >
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Card title="Revenue and User Growth Chart" bordered={false} style={{ marginTop: '50px'}}>
