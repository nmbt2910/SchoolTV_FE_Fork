    ...
    useEffect(() => {
        const fetchChannelData = async () => {
            ...
            try {
                ...
                params.append("accountId", JSON.parse(localStorage.getItem('userData')).accountID);
                const response = await apiFetch(`/api/schoolchannels/search?${params.toString()}`);
    ...
    const handleUpdateChannel = async (values) => {
        try {
            const changedFields = Object.keys(values).reduce((acc, key) => {
                if (String(values[key]).trim() !== String(initialValuesRef.current[key]).trim()) {
                    acc[key] = values[key];
                }
                return acc;
            }, {});
            ...
            const response = await apiFetch(`/api/schoolchannels/${channel.schoolChannelID}`, {
                method: 'PUT',
                ...
                <div className="studio-channel-header-content">
                    <div className="studio-channel-avatar-wrapper">
                        <div className="studio-channel-avatar">
                            <img
                                src={`https://ui-avatars.com/api/?name=${encodeURIComponent(channel.name)}&size=200&background=random`}
                                alt={channel.name}
                            />
                    ...
                    <div className="studio-channel-main-info">
                        ...
                        <p className="studio-channel-bio">{channel.description || "Chưa có mô tả cho kênh này"}</p>
            ...
            >
                ...
                >
                    ...
                    <div className="studio-channel-modal-footer-info">
                        ℹ️ Cập nhật thông tin kênh giúp người dùng dễ dàng tìm thấy và tương tác với nội dung của bạn.
                    </div>
                </Form>
            </Modal>
        </div>
